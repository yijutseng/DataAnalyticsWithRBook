...
## 函數命名原則
根據上述兩個簡單的函數，可以發現寫好的函數的方法是先確保城市的正確性，再確保函數是否容易閱讀與理解，為了達成函數的易理解性，函數有以下建議的命名原則:
...
以計算加權成績為例，我們有一組學生的國文成績chi_score以及英文成績eng_score，並且希望將這些成績以（國文成績+2*英文成績）的方式做加權計算，首先，我們須先將加權的程式碼撰寫成`weight_score()`函數，並使用`map2`家族函數，完成逐一計算的任務。`map2`家族函數的使用方法為`map2_dbl(需逐一計算的向量1,需逐一計算的向量2,計算所需的函數)`，如下列程式碼：
```{r}
